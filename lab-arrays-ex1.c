...
int main(){
   ...
   int c[5] = {0,2,3,4,5,6}  
   /*
   lab-arrays-ex1.c:14:4: warning: excess elements in array initializer
   Nieprawidłowy rozmiar tablicy c (powinno być 6 elementów, a nie 5 - 
   próbujemy przepisać zbyt dużo elementów do tablicy c.
   */
   int *ptr = a;
   /*
   lab-arrays-ex1.c:20:4: error: expected ‘,’ or ‘;’ before ‘int’
   Nie ma na końcu poprzedniej linii średnika (lub przecinka).
   */
  
   a = b;
   /*
   lab-arrays-ex1.c:26:6: error: assignment to expression with array type
   'a' jest adresem tablicy, więc nie można go modyfikować.
   */
   b = ptr;	
   /*
   lab-arrays-ex1.c:31:8: error: ‘ptr’ undeclared (first use in this function)
